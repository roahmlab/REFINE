...
Bounding parameters $M_u$, $M_v$, and $M_r$ are selected to be the maximum value of $|\Delta_u(t)|$, $|\Delta_v(t)|$, and $|\Delta_r(t)|$ respectively over all time, abd $b_u^\text{pro}$ and $b_u^\text{off}$ are generated by bounding $|\Delta_u(t)|$ from above when $u(t) \leq u^\text{cri}$. R.5 provides an illustration of the modeling error along the dynamics of longitudinal velocity.

<figure>
<p align="center">
  <img height="300" src="../Image/delta_u.png"/>
  <figcaption> <i> Fig. R.5 - An illustration of the modeling error along the dynamics of $u$. Collected $\Delta_u(t)$ is bouunded by $M_u = 1.11$ for all time. Whenever $u(t) \leq u^\text{cri} = 0.5$, $\Delta_u(t)$ is bounuded by $b_u^\text{pro}u(t) + b_u^\text{off}$ with $b_u^\text{pro} = 1.2$ and $b_u^\text{off} = 0.51$. </i> </figcaption>
</p>
 </figure>
