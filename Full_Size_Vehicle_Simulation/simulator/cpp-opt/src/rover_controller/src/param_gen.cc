...
#pragma omp parallel for  // JL: serial for debugging
    for (int app_idx = 0; app_idx < num_apps; ++app_idx) {
      ...
      // TODO REMOVE for (auto val : obs_info_to_use.obs_centers_) {
      // TODO REMOVE   array_msg.data.push_back(val);
      // TODO REMOVE }
      for (std::size_t i = 0; i < obs_info_to_use.GetNumObs(); ++i) {
        const ::roahm::Interval T_TMP{0.0, 0.0}; // TODO this seciton is bad
        const auto [x_tmp, y_tmp] = obs_info_to_use.GetCenter(i, T_TMP);
        array_msg.data.push_back(x_tmp);
        array_msg.data.push_back(y_tmp);
      }
      ...
      Ipopt::SmartPtr<Ipopt::TNLP> mynlp =
          new fl_zono_ipopt_problem::FlZonoIpoptProblem(
              ...
              cons.zono_obs_sizes_, k_rng, frs_select_info.manu_type_);
