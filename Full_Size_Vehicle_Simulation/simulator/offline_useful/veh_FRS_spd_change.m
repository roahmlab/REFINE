...
%         delAu = zeros(dim,1); delAu(11) = 1; %this is ignored during lane change, but is helpful during braking
        delv0 = zeros(dim,1);  delr0 = zeros(dim,1); 
        if u0 > 5 && Au > 5
            delv0(5) = v0_limit_gen(t0_idx,del_y_idx,u0_idx); delv0(8) = v0_limit_gen(t0_idx,del_y_idx,u0_idx);
            delr0(6) = r0_limit_gen(t0_idx,del_y_idx,u0_idx); delr0(9) = r0_limit_gen(t0_idx,del_y_idx,u0_idx);
        end
...
%             
%             if u0 == 5 
%                 brake_time = (Au+1-5)/acc+0.3 + 0.5;
%             elseif u0 == 7
%                 brake_time = (Au+1-5)/acc+0.3;
%             else 
...
%             end
       ...
       %% Sean's low speed model
        ...
        save(".\data\spd_change\spd_change_u="+num2str(u0)+"_Au="+num2str(Au)+".mat",'vehRS_save','JL_WARNING');
...
% % end
% load("lane_change_u=5_Ay=-0.23584.mat"); % I only have this one in the repo, genearte your own will take 90 seconds each
% load_const;
% u0 = 5;
% Au = u0;
% t0 = 2;
...
%     zonoslice =vehRS_save{i}{1};%zonotope_slice(vehRS_save{i}{1}, [7;8;9;12], [u0;v0;r0;del_y]);
