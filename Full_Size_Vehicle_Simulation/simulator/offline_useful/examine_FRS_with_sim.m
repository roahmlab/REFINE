...
FRS_zero_ini = load("./data/dir_change/various_initial_condition/dir_change_t0=0_u=1_Ay=2,0.45.mat"); % I only have this one in the repo, genearte your own will take 90 seconds each
FRS_vary_ini = load("./data/dir_change/zero_initial_condition/dir_change_t0=0_u=1_Ay=2,0.45.mat"); % I only have this one in the repo, genearte your own will take 90 seconds each
...
for j = 1:2
    if j == 1
        ...
        FRS_file = FRS_zero_ini;
    else
        
        FRS_file = FRS_vary_ini;
    end
    subplot(2,2,j*2-1);hold on
...
%     p_FRS.EdgeAlpha = 0.5;
    ...
    slicewithftobs = slicewithft+localize_error;
    slicewithftobs = slicewithft;
    
%     p_FRS = plotFilled(slicewithft, [1, 2], 'm');
%     p_FRS.FaceAlpha = .2;
%     p_FRS.EdgeAlpha = 0.5;
    ...
    lon_gen = max(0, predict_err.lon_err_dir_max(error_idx));
    lat_gen = max(abs(predict_err.lat_err_dir_min(error_idx)), predict_err.lat_err_dir_max(error_idx));
    head_gen = max(abs(predict_err.head_err_dir_min(error_idx)), predict_err.head_err_dir_max(error_idx));
%     lon_gen =0.001;
%     lat_gen =0.001;
%     head_gen = 0.001;
    ...
    slicewithft_predict_err = slicewithftobs+predict_err_zono;
...
%     ylabel(convertStringsToChars(num2str(j)+" Ay bin"))
    ...
    grid(gca,'minor')
    xlim([-0.3,3]);
    title('Green: Sliced + Errors')

    
end
subplot(2,2,1);
ylabel('With previous plan Error')
subplot(2,2,3);
ylabel('No previous plan Error')
for car_idx = 1:4
    subplot(2,2,car_idx)
