...
for t0_idx = 1:length(t0_arr)
    ...
    for u0 = u0_vec % set range of Ay or del_y allowed
        ...
        for del_y_idx = 1:length(del_y_arr)
            ...
            %         JL_WARNING = 0;
            t_prev = zeros(dim,1); t_prev(end)=tpk_dir-t0+brake_time;%assume it complete the whole time
            options.R0 = deleteZeros(Rt{end}{1}.set)- t_prev;
            options.x0 = center(options.R0);
            ...
            for i = 1:length(vehBrk_low)
                ...
                temp = zeros(dim,1); temp(end) = tpk_dir-t0+brake_time;
            ...
            if previous_error_flag
                save("./data/dir_change/various_initial_condition/dir_change_t0="+num2str(t0)+"_u="+num2str(u0)+"_Ay="+num2str(del_y_idx)+","+num2str(del_y)+".mat",'vehRS_save');
            else
                save("./data/dir_change/zero_initial_condition/dir_change_t0="+num2str(t0)+"_u="+num2str(u0)+"_Ay="+num2str(del_y_idx)+","+num2str(del_y)+".mat",'vehRS_save');
            end
        end
    end
end