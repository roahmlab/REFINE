classdef JL_highwayAgentHelper_Nopt < agentHelper
    %% properties
    properties
...
%         zono_dt ;
    ...
    %% methods
    methods
        function AH = JL_highwayAgentHelper_Nopt(A,FRS_obj,HLP,varargin)
...
%                 dAu = 1000;
                ...
                for idx = 1:length(dAu)
                    ...
                    FRS = MAu(type_text);
                    [K,cost_val,save_pkg] = AH.highway_sampling_opt( x_des, FRS, all_tb{type_manu}, idx, O,dyn_O, agent_state, type_manu, AH.cur_t0_idx, traffic_data,waypts);
                    if ~isempty(K)
                ...
                if abs(x_des(2) - agent_state(2))>0.8 || isempty(all_K)
                    %%% dir change
                    type_manu = 2;
        ...
        function [K,cost_val, save_pkg]= highway_sampling_opt(AH, x_des, FRS,tbdir,diridx_with_mirror,O, dyn_O, agent_state, type_manu,t0_idx, traffic_data,wayypts)
...
%                         end
            ...
            if ~isempty(K)
                ...
                save_pkg.waypt = reshape(wayypts,[9,1]);
                save_pkg.time = AH.A.time(end);
            end
            %             end
        end
