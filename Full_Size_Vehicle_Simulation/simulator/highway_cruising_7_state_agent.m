classdef highway_cruising_7_state_agent < RTD_agent_2D
    ...
    methods
        %% constructor
        function A = highway_cruising_7_state_agent(varargin)
...
%             A.Kw = Kw;
        ...
        function [delta, w_cmd] = FL_LLC(A,t,z,T,U,Z)
            u = z(4);
            v = z(5);
            r = z(6);
            w = z(7);
...
%             v_dot_des = A.Kv*(vd -v) + vddot + A.Kr *(rd -r);
            ...
            alphar = atan( uv_wr(2) ./ (uv_wr(1) + sign_pos_neg(uv_wr(1))*0.2));
            
            %             Fxwr = A.Cr1*tanh(A.Cr2*betar);
            Fywr = -A.Car1*tanh(A.Car2*alphar);
            
            delta_arr = linspace(-0.35,0.35);
...
%                 betaf = (w - uv_wf(1)) ./ max(sqrt(uv_wf(1).^2 +0.05), sqrt(w.^2 +0.05));
                ...
                alphaf = atan( uv_wf(2) ./ (uv_wf(1) + sign_pos_neg(uv_wf(1))*0.2));
...
%             desired_front_lat_force = A.m*(v_dot_des + u*r) - Fywr; % v control
              desired_front_lat_force = (A.Izz*(r_dot_des) + A.lr*Fywr)/A.lf;
%             
...
%             w_cmd = A.Kw*(w_hat - w) + w ;
            ...
            w_cmd = ((u_dot_des - v*r)*A.m + sin(delta)*Fywf)/(A.Cw*ratio_term) + w;
%             if w_cmd < 0.5
%                 a = 1;
%             end
...
%             u = w;
            ...
            [delta, w_cmd] = A.FL_LLC(t,z,T,U,Z);
%             w_cmd
%             [w_cmd, u]
