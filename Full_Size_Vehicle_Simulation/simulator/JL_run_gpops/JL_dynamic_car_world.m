classdef JL_dynamic_car_world < world
    properties
        buffer = 1 ;
        obstacles_unseen
        obstacles_seen
    ...
    methods
        %% constructor
        function W = JL_dynamic_car_world(envCars,varargin)
        ...
        %             end
        %         end
        function setup(W,envCars)
            if ~exist('envCars','var')
                envCars = W.envCars;
            end
            W.envCars = envCars;
%             W.placeBlockedCars();
        ...
        function placeCars(W)
            ...
            MAX_DIST =  abs(W.bounds(2)) - W.car_safe_dist;
            for i = 2:W.num_cars
                laneOverlap = true;
                while laneOverlap
                    xPos = MAX_DIST * rand  + 50;% start at least 50
                    laneNum = randi([0,2]);
                    laneOverlap = false;
                    for j = 1:i
                    ...
                    if ~laneOverlap % assign car to new slot
                        W.envCars(i,1) = xPos;
                        if i <= W.num_cars - W.num_moving_cars
                            W.envCars(i,2) = 0;
                        else
                            W.envCars(i,2) = W.car_min_spd+rand*(W.car_max_spd-W.car_min_spd);
                        end
                        W.envCars(i,3) = W.lanewidth*laneNum ;
                        W.envCars(i,4) = W.car_max_spd-rand*(W.car_max_spd-W.car_min_spd)/3;
...
%             O_plot = nan(2,6*N_obs);
%             O = W.obstacles;
%             O_future = W.obstacles;
            for idx = 1:6:(6*N_obs-1)
                c = [W.envCars(obs_count,1); W.envCars(obs_count,3)];
...
%                 O_plot(:,idx:idx+4) =  W.o_no_frs + repmat(c,1,5) + [(agent_info.time(end))*W.envCars(obs_count,2);0];
                %                         [~,v_idx] = min(abs(W.v_array-W.envCars(obs_count,2)));
                %                         for t_idx = 1:30
                %                             W.obs_FRS{t_idx}(:,idx:idx+4) = W.o {v_idx,t_idx} + repmat(c,1,5) ;
                %                         end
...
%             obs_time = 
%             W.dynamics_obstacles = O_dynamic;
%             W.dynamics_obstacles_time = ;
            W.obstacles_seen = O;
            W.obstacles_old = O;
...
%             obs_count = 2;
            for idx = 1:6:(6*N_obs-1)
                c = [W.envCars(obs_count,1); W.envCars(obs_count,3)];
%                 O_future(:,idx:idx+4) = W.o_no_frs + repmat(c,1,5) + [(agent_info.time(end))*W.envCars(obs_count,2);0];
